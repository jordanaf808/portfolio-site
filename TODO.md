...
[ ] - Confirm where production builds run before next deploy. If Cloudflare Pages builds on its Linux CI, it needs the Linux Sharp binary (@img/sharp-linux-x64 or -linux-arm64); only @img/sharp-darwin-arm64 is installed locally. If instead you build locally and deploy dist/, no action needed. Relevant because astro.config.mjs uses imageService 'compile' (build-time Sharp) for production; dev uses 'passthrough'.
