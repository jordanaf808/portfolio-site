...
- Corrected the surrounding context for Workers in `security.md`: the Rate Limiting section now documents the native Workers `ratelimits` binding (Workers _does_ have built-in rate limiting); the Headers section now notes that `_headers` applies only to static-asset responses, not the Worker-rendered `/api/contact`.
...
- `api/contact.ts`: verifies the token against Cloudflare `siteverify` (with `remoteip` from `clientAddress`) _before_ validation/Resend; a missing or failing token returns `403` and never reaches the mailer.
...
**Why:** Client-side constraint hints give instant feedback and stop an over-long or too-short payload before a wasted round-trip. These are HTML attributes that _mirror_ the server schema (`src/lib/contactSchema.ts`) — not a second validation layer — so there's no duplicated logic to drift. The server remains the security boundary.
